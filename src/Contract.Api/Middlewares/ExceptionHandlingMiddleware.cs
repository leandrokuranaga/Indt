...
using Contract.Application.Validators;
...
public class ExceptionHandlingMiddleware(RequestDelegate next, ILogger<ExceptionHandlingMiddleware> logger)
{
    public async Task Invoke(HttpContext context)
    {
        ...
        catch (NotFoundException e)
        {
            ...
            await HandleExceptionAsync(context, e, "Not Found", NotificationModel.ENotificationType.NotFound,
                HttpStatusCode.NotFound);
        }
        catch (ValidatorException e)
        {
            await RollbackIfPossibleAsync(context);
            await HandleExceptionAsync(context, e, "Validation Error",
                NotificationModel.ENotificationType.BadRequestError, HttpStatusCode.BadRequest);
        }
        catch (BusinessRulesException e)
        {
            await RollbackIfPossibleAsync(context);
            await HandleExceptionAsync(context, e, "Business Rules", NotificationModel.ENotificationType.BusinessRules,
                HttpStatusCode.Conflict);
        }
        catch (Exception e)
        {
            await RollbackIfPossibleAsync(context);
            await HandleExceptionAsync(context, e, "Internal Error",
                NotificationModel.ENotificationType.InternalServerError, HttpStatusCode.InternalServerError);
        }
    }
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception, string title,
        NotificationModel.ENotificationType type, HttpStatusCode statusCode)
    {
